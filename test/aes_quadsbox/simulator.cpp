...
enum Opcode {
  ...
  OP_CUSTOM1 = 0x2B // Custom-1: AES S-Box / Inverse S-Box lookup
};
...
bool computer(uint32_t imem_arg[MEM_SIZE], uint32_t dmem_arg[MEM_SIZE]
  ...
  while (!halt && PC < MEM_SIZE * 4) {
    ...
    // ================================================================
    // CUSTOM-1: AES S-Box Lookup (R-type)
    // funct3=0: SBOX   rd = aes_sbox[rs1 & 0xFF]
    // funct3=1: INVSBOX rd = aes_inv_sbox[rs1 & 0xFF]
    // ================================================================
    case OP_CUSTOM1: {
      uint32_t op1 = regs[rs1];
      uint32_t result = 0;
      if (funct3 == 0x0) {
        // Single-byte SBOX
        result = aes_sbox[op1 & 0xFF];
      } else if (funct3 == 0x1) {
        // Single-byte INVSBOX
        result = aes_inv_sbox[op1 & 0xFF];
      } else if (funct3 == 0x2) {
        // QUAD_SBOX: 4 packed bytes through S-box in parallel
        result = (uint32_t)aes_sbox[op1 & 0xFF] |
                 ((uint32_t)aes_sbox[(op1 >> 8) & 0xFF] << 8) |
                 ((uint32_t)aes_sbox[(op1 >> 16) & 0xFF] << 16) |
                 ((uint32_t)aes_sbox[(op1 >> 24) & 0xFF] << 24);
      } else if (funct3 == 0x3) {
        // QUAD_INVSBOX: 4 packed bytes through inverse S-box in parallel
        result = (uint32_t)aes_inv_sbox[op1 & 0xFF] |
                 ((uint32_t)aes_inv_sbox[(op1 >> 8) & 0xFF] << 8) |
                 ((uint32_t)aes_inv_sbox[(op1 >> 16) & 0xFF] << 16) |
                 ((uint32_t)aes_inv_sbox[(op1 >> 24) & 0xFF] << 24);
      }
      if (rd != 0)
        regs[rd] = result;
      break;
