...
void decode_block(int comp_no, int *out_buf, int *HuffBuff) {
  ...
  p_quant_tbl =
      &p_jinfo_quant_tbl_quantval[(int)p_jinfo_comps_info_quant_tbl_no[comp_no]]
                                 [DCTSIZE2];
  ...
  *out_data_image_height = p_jinfo_image_height;
  ...
  if (p_jinfo_smp_fact == SF1_1_1) {
    ...
    while (CurrentMCU < p_jinfo_NumMCU) {
      /*
       * Decode Y element
       * Decoding Y, U and V elements should be sequentially conducted for the
       * use of Huffman table
       */
      ...
      for (i = 0; i < RGB_NUM; i++) {
        Write4Blocks(&rgb_buf[0][i][0], &rgb_buf[1][i][0], &rgb_buf[2][i][0],
                     &rgb_buf[3][i][0], &out_data_comp_vpos[i],
                     &out_data_comp_hpos[i], &OutData_comp_buf[i][0]);
      }
