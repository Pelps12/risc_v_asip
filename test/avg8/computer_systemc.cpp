...
SC_MODULE(rv32i_core) {
    ...
    void mem_write_byte(uint32_t addr, uint8_t val) {
        ...
        if (offset < MEM_SIZE * 4) {
            ...
            uint32_t bit_offset = (offset & 3) * 8;
            uint32_t mask = ~(0xFFu << bit_offset);
            dmem[word_addr] = (dmem[word_addr] & mask) | ((uint32_t)val << bit_offset);
        }
    }

    // AVE8 accelerator: running average of 8 bytes
    uint32_t compute_avg8(uint32_t in0) {
        // Shift data to accommodate new input
        for (int i = 7; i > 0; i--) {
            avg8_buffer[i] = avg8_buffer[i - 1];
        }
        // Read new input into buffer
        avg8_buffer[0] = in0 & 0xFF; // Treat as byte

        // Sum up all the numbers in the buffer
        uint32_t sum = 0;
        for (int i = 0; i < 8; i++) {
            sum += avg8_buffer[i];
        }
        // Compute the average by dividing by 8
        return sum / 8;
    }
        ...
        while (true) {
            ...
            switch (opcode) {
                ...
                case OP_LOAD: {
                    ...
                    if (rd != 0) {
                        switch (funct3) {
                            case 0x0: regs[rd] = (int32_t)(int8_t)mem_read_byte(addr); break;   // LB
                            case 0x1: {                                                       // LH
                                uint32_t low = mem_read_byte(addr);
                                uint32_t high = mem_read_byte(addr + 1);
                                regs[rd] = (int32_t)(int16_t)(low | (high << 8));
                                break;
                            }
                            case 0x2: regs[rd] = mem_read_word(addr); break;                  // LW
                            case 0x4: regs[rd] = mem_read_byte(addr); break;                  // LBU
                            case 0x5: {                                                       // LHU
                                uint32_t low = mem_read_byte(addr);
                                uint32_t high = mem_read_byte(addr + 1);
                                regs[rd] = low | (high << 8);
                ...
                case OP_STORE: {
                    ...
                    switch (funct3) {
                        case 0x0: mem_write_byte(addr, regs[rs2] & 0xFF); break;              // SB
                        case 0x1:                                                             // SH
                            mem_write_byte(addr, regs[rs2] & 0xFF);
                            mem_write_byte(addr+1, (regs[rs2] >> 8) & 0xFF);
                            break;
                        case 0x2: mem_write_word(addr, regs[rs2]); break;                     // SW
                    }
                    break;
                }
    ...
    SC_CTOR(rv32i_core) {
        ...
        for(int i=0; i<8; i++) { avg8_buffer[i] = 0; }
    }
};
