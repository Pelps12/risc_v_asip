...
inline uint32_t decode_imm_u(uint32_t instr) {
...
void load_program(const string& filename) {
    ...
    while (getline(file, line)) {
        ...
        if (line[0] == '@') {
            ...
            if (addr < MEM_SIZE) {
                imem[addr] = val;
            } else if (addr >= MEM_SIZE && addr < 2 * MEM_SIZE) { // Mapping address 40000h to dmem[0]
